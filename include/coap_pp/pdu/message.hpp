...
struct Code {
  ...
  [[nodiscard]] constexpr uint8_t ClassBits()  const noexcept { return (value >> 5u) & 0x07u; }
  [[nodiscard]] constexpr uint8_t DetailBits() const noexcept { return value & 0x1Fu; }
...
// Fully deserialized CoAP message. All spans are non-owning views into the original
// datagram buffer — the buffer must outlive this struct.
struct Message {
