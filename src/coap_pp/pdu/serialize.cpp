...
// Returns the minimum number of bytes needed to represent v as a big-endian uint.
// Returns 0 for v == 0 (RFC 7252 §3.2 zero-length encoding).
...
SerializeError Serialize(const OutgoingMessage& msg,
  ...
  if (!WriteByte(out, pos, static_cast<uint8_t>(0x40u | (type_bits << 4u) | tkl)) ||
  ...
  for (const auto& opt : msg.options) {
    ...
    const std::size_t value_len = std::visit(overloaded{
      ...
      [](std::span<const std::byte> s)       -> std::size_t { return s.size(); },
    }, opt.value);
    ...
    if (!EncodeExtField(static_cast<uint32_t>(value_len), out, pos, length_nibble)) {
      return SerializeError::kBufferTooSmall;
    }

    // Write header byte now that both nibbles are known
    out[header_pos] = static_cast<std::byte>((delta_nibble << 4u) | length_nibble);

    // Write value bytes
    const bool write_ok = std::visit(overloaded{
      [](std::monostate) { return true; },
      [&](uint32_t v) {
        if (v == 0u) return true;
        const uint8_t len = UintEncodedLength(v);
        for (int shift = static_cast<int>(len - 1u) * 8; shift >= 0; shift -= 8) {
          if (!WriteByte(out, pos,
                         static_cast<uint8_t>((v >> static_cast<unsigned>(shift)) & 0xFFu))) {
            return false;
          }
        }
        return true;
