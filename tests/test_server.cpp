...
TEST_F(ServerTest, Dispatch_KnownPath_CallsHandler) {
  bool called = false;
  const std::array<Route, 1> routes{{{codes::kGet, "/temp", [&](const Request&) -> Response {
    called = true;
...
TEST_F(ServerTest, Dispatch_UnknownPath_Returns404) {
  const std::array<Route, 1> routes{{{codes::kGet, "/known", [](const Request&) -> Response {
    return {codes::kContent, {}};
  }}}};
  Router router{"", routes};
  server_.AddRouter(router);
...
TEST_F(ServerTest, Dispatch_UnknownMethod_Returns405) {
  const std::array<Route, 1> routes{{{codes::kGet, "/res", [](const Request&) -> Response {
    return {codes::kContent, {}};
  }}}};
  Router router{"", routes};
  server_.AddRouter(router);

  // Code 0.05 is not a standard RFC 7252 method — path matches, method does not.
...
TEST_F(ServerTest, Dispatch_WrongMethod_Returns405_WithoutCallingHandler) {
  // Route only accepts GET; a POST must yield 4.05 automatically — handler not called.
...
TEST_F(ServerTest, Dispatch_EmptyCode_Ignored) {
  ...
  // Code 0.00 = Empty (ping/ACK) — must be silently dropped before route lookup.
...
TEST_F(ServerTest, CON_Request_GetsACKResponse) {
  const std::array<Route, 1> routes{{{codes::kGet, "/r", [](const Request&) -> Response {
    return {codes::kContent, {}};
  }}}};
  Router router{"", routes};
  server_.AddRouter(router);
...
TEST_F(ServerTest, HandlerReceives_Method_And_Payload) {
  ...
  const std::array<Route, 1> routes{{{codes::kPost, "/ep", [&](const Request& req) -> Response {
...
TEST_F(ServerTest, Response_ContentFormat_AddedWhenSet) {
  const std::array<Route, 1> routes{{{codes::kGet, "/data", [](const Request&) -> Response {
...
TEST_F(ServerTest, Response_NoContentFormat_WhenNotSet) {
  const std::array<Route, 1> routes{{{codes::kGet, "/plain", [](const Request&) -> Response {
...
TEST_F(ServerTest, MultipleResources_EachDispatches) {
  ...
  const std::array<Route, 2> routes{{
      {codes::kGet, "/temp",  [&](const Request&) -> Response { ++temp_count;  return {}; }},
      {codes::kGet, "/humid", [&](const Request&) -> Response { ++humid_count; return {}; }},
  }};
  Router router{"", routes};
  server_.AddRouter(router);
...
TEST_F(ServerTest, MultiSegmentPath_Dispatches) {
  bool called = false;
  const std::array<Route, 1> routes{{{codes::kGet, "/sensors/temperature",
...
TEST_F(ServerTest, MultiSegmentPath_DoesNotMatchPrefix) {
  bool called = false;
  const std::array<Route, 1> routes{{{codes::kGet, "/sensors",
      [&](const Request&) -> Response {
        called = true;
        return {codes::kContent, {}};
      }}}};
...
TEST_F(ServerTest, RouterBasePath_DispatchesWithPrefix) {
  bool called = false;
  const std::array<Route, 1> routes{{{codes::kGet, "/temperature",
      [&](const Request&) -> Response {
        called = true;
        return {codes::kContent, {}};
      }}}};
...
TEST_F(ServerTest, MultipleRouters_EachDispatches) {
  ...
  const std::array<Route, 1> sensor_routes{{{codes::kGet, "/temp",
      [&](const Request&) -> Response {
        sensors_called = true;
        return {codes::kContent, {}};
      }}}};
  const std::array<Route, 1> actuator_routes{{{codes::kPut, "/led",
      [&](const Request&) -> Response {
...
TEST_F(ServerTest, Async_NON_NoSendBeforeAsyncResponseSendCalled) {
  ...
  const std::array<Route, 1> routes{{{codes::kGet, "/slow",
      [&pending](const Request& req) -> HandlerResult {
