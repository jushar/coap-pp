...
TEST(Serialize, RoundTrip_WithOptionsAndPayload) {
  ...
  Message in_msg{};
  ASSERT_EQ(Deserialize(std::span<const std::byte>{buf.data(), size}, in_msg), DeserializeError::kOk);
...
TEST(MessageBuilder, ChainSetters) {
  MessageBuilder<4> b;
  b.SetType(MessageType::kNon)
   .SetCode(codes::kPost)
   .SetMessageId(0x1234u);
...
TEST(MessageBuilder, SortsOptions) {
  ...
  b.SetType(MessageType::kCon)
   ...
   .AddOption(11u, std::string_view{"path"});  // added second but lower number
...
TEST(MessageBuilder, AllOptionTypes) {
  ...
  EXPECT_TRUE(std::holds_alternative<std::span<const std::byte>>(msg.options[0].value));
